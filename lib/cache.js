...
const checkArchitecture = (filename) => {
  if (filename.includes('arm64')) return 'arm64';
  if (filename.includes('x64') || filename.includes('Setup.exe') || filename.includes('full.nupkg')) return 'x64'; // electron forge names the exe file ...Setup.exe and the nupkg file ...full.nupkg
...
module.exports = class Cache {
  ...
  async refreshCache() {
    ...
    for (const asset of release.assets) {
      ...
      const architecture = checkArchitecture(name);
      ...
      this.latest.platforms[`${platform}-${architecture}`] = entry; // Store based on platform and architecture
  ...
  async loadCache() {
    ...
    console.log('latest from cache', JSON.stringify(latest, null, 2))
