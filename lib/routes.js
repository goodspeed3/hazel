...
module.exports = ({ cache, config }) => {
  ...
  // defaults to downloading the arm64 version
  exports.download = async (req, res) => {
    ...
    if (userAgent.isMac && isUpdate) {
      platform = 'darwin'
      platformAndArch = 'darwin-arm64'
    } else if (userAgent.isMac && !isUpdate) {
      platform = 'dmg'
      platformAndArch = 'dmg-arm64'
    } else if (userAgent.isWindows && isUpdate) {
      platform = 'exe'
      platformAndArch = 'nupkg-x64'
    } else if (userAgent.isWindows && !isUpdate) {
      platform = 'exe'
      platformAndArch = 'win32-x64'
    }

    // Get the latest version from the cache
    const { platforms } = await loadCache()

    if (!platform || !platforms || !platforms[platformAndArch]) {
      send(res, 404, 'No download available for your platform!')
      return
    }

    if (shouldProxyPrivateDownload) {
      proxyPrivateDownload(platforms[platformAndArch], req, res)
      return
    }

    res.writeHead(302, {
      Location: platforms[platformAndArch].url
    })
  ...
  exports.update = async (req, res) => {
    ...
    } else if (platform === 'exe-x64') {
      platform = 'nupkg-x64'
      platformName = 'nupkg-x64'
    }
    const latest = await loadCache()
    if (!latest.platforms || !latest.platforms[platform]) {
      res.statusCode = 204
    ...
    if (compare(latest.version, version) !== 0) {
      ...
      console.log(responseObj, {shouldProxyPrivateDownload})
      console.log(`Update available: ${latest.version} from ${version}`)
      send(res, 200, responseObj)
